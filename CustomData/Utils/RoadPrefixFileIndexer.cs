...
namespace CustomData.Utils
{
    internal class RoadPrefixFileIndexer
    {
        ...
        public string GetPrefix(PrefabAI ai, bool isOneWay, bool isSymmetric, float width, byte lanes, Randomizer rand, ushort segmentId, ushort seedId)
        {
            ...
            var seedConf = CDStorage.Instance.InstanceExtraData.TryGetValue(new InstanceID { Type = (InstanceType)InstanceIdUtils.TYPE_CD_HIGHWAYINSTANCE, Index = seedId }.RawData, out var seedConfBase) ? new HighwayInstanceDW(seedConfBase) : null;
            SeedConfiguration seedConfiguration =
                (!(seedConf?.Type is null) ? SeedConfiguration.HAS_HIGHWAY_TYPE : SeedConfiguration.NO_HIGHWAY_TYPE)
                | ((seedConf?.ForceName).IsNullOrWhiteSpace() ? SeedConfiguration.NO_FORCED_NAME : SeedConfiguration.HAS_FORCED_NAME);
            bool hasStart = true;
            bool hasEnd = true;
            if (wayVal == OneWay.TRUE && lanes <= 2)
            {
                GetSegmentRoadEdges(segmentId, true, true, true, out ComparableRoad startRef, out ComparableRoad endRef, out _, out _, out _);
            ...
            return filterResult?.Count == 0 ? null : filterResult[rand.Int32((uint)(filterResult.Count))].name;
        }
