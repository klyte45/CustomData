...
{
    public class NetManagerOverrides : Redirector, IRedirectable
    {
        ...
        public static bool GenerateSegmentNameInternal(ushort segmentID, ref string __result, ref List<ushort> usedQueue, bool removePrefix)
        {
            ...
            string format = GetRoadNameFormat(segmentID, removePrefix, ref segment, info, ai, out byte district);
            ...
            if (format.Contains("{1}") || format.Contains("{2}") || format.Contains("{3}") || format.Contains("{4}") || format.Contains("{7}"))
            {
                ...
                if (format.Contains("{7}"))//direction
                {
                    ...
                    //}
                    var cardinalDirection = SegmentUtils.GetCardinalDirection(startRef, endRef, axis);

                    variables.direction = ((Cardinal16)(2 * cardinalDirection.GetCardinalIndex8())).ValueToI18n();
                }
            }
        ...
        internal static string GetRoadNameFormat(ushort segmentID, bool removePrefix, ref NetSegment segment, NetInfo info, PrefabAI ai, out byte district)
        {
            string format = null;
            var randomizer = new Randomizer(segment.m_nameSeed);
            district = (byte)(DistrictManager.instance.GetDistrict(segment.m_middlePosition) & 0xFF);
            var districtConfig = CDStorage.Instance.GetDistrictData(district);
            var cityConfig = district == 0 ? null : CDStorage.Instance.GetDistrictData(0);

            if ((info.m_vehicleTypes & VehicleInfo.VehicleType.Car) != VehicleInfo.VehicleType.None)
            {
                string filenamePrefix = districtConfig.RoadQualifierFile;
                ;
                if (cityConfig != null && (filenamePrefix == null || !CDController.LoadedRoadPatterns.ContainsKey(filenamePrefix)))
                {
                    filenamePrefix = cityConfig.RoadQualifierFile;
                }

                if (filenamePrefix != null && CDController.LoadedRoadPatterns.TryGetValue(filenamePrefix, out var currentPrefixFile))
                {
        ...
        private static void GetGeneratedRoadName(ref NetSegment segment, PrefabAI ai, byte district, out string genName)
        {
            string filename = CDStorage.Instance.GetDistrictData(district).RoadNamesFile;
            if (filename.IsNullOrWhiteSpace() || !CDController.LoadedGeneralNames.ContainsKey(filename))
            {
                filename = CDStorage.Instance.GetDistrictData(0).RoadNamesFile;
                if (filename.IsNullOrWhiteSpace() || !CDController.LoadedGeneralNames.ContainsKey(filename))
                {
                    filename = null;
                }
            ...
            if (filename != null)
            {
                int range = CDController.LoadedGeneralNames[filename]?.Length ?? 0;
                if (range == 0)
                {
                    genName = null;
                    return;
                }
                genName = CDController.LoadedGeneralNames[filename][randomizer.Int32((uint)range)];
            }
            else
            {
